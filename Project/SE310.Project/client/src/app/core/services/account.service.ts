...
export class AccountService {
  ...
  getForgotPassword(email: string) {
    return this.http.get(this.baseUrl + 'account/send-mail/' + email, {
      responseType: 'text',
    });
  }

  getGoogleState() {
    return this.http.get<{ isGoogle: boolean }>(
      this.baseUrl + 'account/is-google'
    );
  }
  ...
  updateUserInfo(values: any) {
    return this.http.post(this.baseUrl + 'account/user-info', values, {
      responseType: 'text',
    });
  }
}
