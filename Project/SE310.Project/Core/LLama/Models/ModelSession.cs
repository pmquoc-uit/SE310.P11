...
namespace Core.LLama.Models
{
    public class ModelSession
    {
        ...
        public async Task InitializePrompt
            (InferenceOptions? inferenceConfig = null, CancellationToken cancellationToken = default)
        {
            ...
            await foreach (
                var _ in _executor.InferAsync
                    (_sessionConfig.Prompt, inferenceParams, _cancellationTokenSource.Token))
            {
        ...
        public IAsyncEnumerable<String> InferAsync
            (String message, InferenceOptions? inferenceConfig = null, CancellationToken cancellationToken = default)
        {
            var inferenceParams = ConfigureInferenceParams(inferenceConfig);
            _cancellationTokenSource = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
            var inferenceStream =
        ...
        public Boolean IsInferCanceled()
        {
